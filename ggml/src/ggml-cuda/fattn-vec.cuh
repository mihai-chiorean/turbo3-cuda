...
static __global__ void flash_attn_ext_vec(
    ...
    constexpr int nthreads_KQ = (type_K == GGML_TYPE_F16 || type_K == GGML_TYPE_BF16 || type_K == GGML_TYPE_TURBO3_0 || type_K == GGML_TYPE_TURBO4_0 || type_K == GGML_TYPE_TBQ4_0) ? 128 / cpy_nb : nthreads_KQ_q;
    constexpr int nthreads_V  = (type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_BF16 || type_V == GGML_TYPE_TURBO3_0 || type_V == GGML_TYPE_TURBO4_0 || type_V == GGML_TYPE_TBQ4_0) ? 128 / cpy_nb : nthreads_V_q;

    static_assert(WARP_SIZE % nthreads_KQ == 0, "bad nthreads_K");
    static_assert(WARP_SIZE % nthreads_V  == 0, "bad nthreads_V");

    constexpr int V_rows_per_thread = (type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_BF16 || type_V == GGML_TYPE_TURBO3_0) ? 2*cpy_ne : 4;
    constexpr int V_cols_per_iter   = WARP_SIZE / nthreads_V;

    constexpr vec_dot_KQ_t vec_dot_KQ = get_vec_dot_KQ<type_K, D, nthreads_KQ>();
    constexpr bool Q_q8_1 = type_K != GGML_TYPE_F16 && type_K != GGML_TYPE_BF16 && type_K != GGML_TYPE_TURBO3_0 && type_K != GGML_TYPE_TBQ4_0;
...
#pragma unroll
    ...
    if constexpr (type_K == GGML_TYPE_TBQ4_0) {
        static_assert(D == 128, "TBQ4_0 only supports head_dim=128");
...
#pragma unroll
            ...
            if (tid < D) {
                for (int k = 0; k < D; ++k)
                    rotated_val += TBQ_ROTATION_128x128[tid * 128 + k] * Q_shared[k];
            }
            __syncthreads();
            if (tid < D) Q_shared[tid] = rotated_val;
            __syncthreads();
#pragma unroll
            for (int i0 = 0; i0 < D/2; i0 += nthreads_KQ*cpy_ne) {
                const int base = i0 + (threadIdx.x % nthreads_KQ)*cpy_ne;
...
#pragma unroll
                ...
                if constexpr (type_V == GGML_TYPE_TBQ4_0) {
                    ...
                    __syncthreads();
                    float rotated = 0.0f;
                    const int out_idx = i0 + tid;
                    for (int i = 0; i < D; ++i)
                        rotated += TBQ_ROTATION_128x128[i * 128 + out_idx] * rot_shared[i];
                    dst_val = rotated;
                    __syncthreads();
                }
                if (gridDim.y == 1) {
